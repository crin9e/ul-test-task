...
describe("AuctionDetailPage", () => {
  it("renders all detail sections and actions", async () => {
    ...
    const routeSection = screen
      .getByRole("heading", { name: "Маршрут" })
      .closest("article");
    expect(routeSection).not.toBeNull();
    const routePoints = within(routeSection!).getAllByRole("listitem");
    expect(routePoints).toHaveLength(4);
    ["Погрузка", "Погрузка", "Выгрузка", "Выгрузка"].forEach(
      (operation, index) => {
        expect(
          within(routePoints[index]).getByRole("heading", { name: operation }),
        ).toBeInTheDocument();
      },
    );
    expect(
  ...
  it("removes protected route and contact data", async () => {
    ...
    expect(
      screen.getByText("Контакты скрыты организатором."),
    ).toBeInTheDocument();
  ...
  it.each([
    ...
    expect(
      await screen.findByRole("heading", { name: title }, { timeout: 3_000 }),
    ).toBeInTheDocument();
  });
});
