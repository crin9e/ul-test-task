...
import { AuctionBetsList } from "./auction-bets-list";
...
export function BetsPage() {
  ...
  const betsQuery = useAuctionBets(params.auctionUuid, search.all, {
    enabled: parsedUuid.success && detailQuery.isSuccess && !historyHidden,
  });
  ...
  return (
    ...
    >
      <nav className={styles.actions} aria-label="Навигация по ставкам">
        ...
        <Link
          className={`secondary outline ${styles.auctionsLink}`}
          role="button"
          to="/auctions/$auctionUuid"
          params={{ auctionUuid: parsedUuid.data }}
        >
