...
export function BidPage() {
  ...
  if (isError) {
    return <ErrorState message={error instanceof Error ? error.message : 'Не удалось открыть форму ставки.'} onRetry={() => void refetch()} />;
  }

  return (
    <PageShell title="Сделать ставку" description="Форма ставки будет добавлена на следующем этапе.">
      <article>
