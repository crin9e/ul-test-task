...
function createValidationResponse(field: string, message: string, code: string) {
...
function includesText(value: string | undefined, query: string | undefined): boolean {
  if (!query) {
    return true;
  }
  return (value ?? '').toLocaleLowerCase('ru-RU').includes(query.toLocaleLowerCase('ru-RU'));
}

function isWithinRange(value: number | null | undefined, from: number | null | undefined, to: number | null | undefined): boolean {
  ...
  return (from === null || from === undefined || value >= from) && (to === null || to === undefined || value <= to);
}

function isWithinDateRange(value: string | undefined, from: string | undefined, to: string | undefined): boolean {
  if (!from && !to) {
    return true;
  }
  if (!value) {
...
function matchesArray<T>(value: T | undefined, accepted: T[] | undefined): boolean {
  return !accepted?.length || (value !== undefined && accepted.includes(value));
}

function matchesListRequest(entry: MockAuctionEntry, request: AuctionListRequest): boolean {
  ...
  const auctionStatusId = detail.trading.status ? auctionStatusIds[detail.trading.status as keyof typeof auctionStatusIds] : undefined;
  const tradingStatusId = detailTradingStatus ? tradingStatusIds[detailTradingStatus as keyof typeof tradingStatusIds] : undefined;
  const customerQuery = request.customer?.toLocaleLowerCase('ru-RU');
  const customerMatches = !customerQuery
    || (organizer?.organization_name ?? '').toLocaleLowerCase('ru-RU').includes(customerQuery)
    || (organizer?.organization_inn ?? '').includes(customerQuery);

  return (
    matchesArray(detailTradingStatus, request.status)
    && (!request.mobile_statuses?.length || (tradingStatusId !== undefined && request.mobile_statuses.includes(tradingStatusId)))
    && (!request.statuses?.length || (auctionStatusId !== undefined && request.statuses.includes(auctionStatusId)))
    ...
    && (!request.body_types?.length || (cargo?.body_type !== undefined && request.body_types.includes(cargo.body_type)))
    && (!request.form_type || payment?.form === request.form_type)
    && (request.is_international_shipment === undefined || cargo?.is_international === request.is_international_shipment)
    && includesText(route?.load?.city, request.load_city)
    && (request.load_gc_id === undefined || route?.load?.city_gc_id === request.load_gc_id)
    && includesText(route?.unload?.city, request.unload_city)
    && (request.unload_gc_id === undefined || route?.unload?.city_gc_id === request.unload_gc_id)
    ...
    && (request.is_available === undefined || trading?.is_available === request.is_available)
    && (request.is_favorite === undefined || trading?.is_favorite === request.is_favorite)
    && (request.is_bidder === undefined || trading?.is_bidder === request.is_bidder)
    && customerMatches
    && (!request.customer_ids?.length || (organizer?.organization_id !== undefined && request.customer_ids.includes(organizer.organization_id)))
    && (!request.auction_ids?.length || (main?.id !== undefined && request.auction_ids.includes(main.id)))
    && isWithinRange(currentPrice, request.current_price_from, request.current_price_to)
    && isWithinRange(pricePerKm, request.price_per_km_from, request.price_per_km_to)
    && (!request.auc_type?.length || (main?.auc_type !== undefined && request.auc_type.includes(main.auc_type as Exclude<typeof main.auc_type, 'Unknown'>)))
  );
}
...
function sortEntries(entries: MockAuctionEntry[], request: AuctionListRequest): MockAuctionEntry[] {
  const supportedSelectors = {
    start_time: (entry: MockAuctionEntry) => entry.listItem.trading?.start_time,
    current_price: (entry: MockAuctionEntry) => entry.listItem.trading?.price?.current,
    price_per_km: (entry: MockAuctionEntry) => entry.listItem.main?.price_per_km,
  } as const;

  const sortRules = Object.entries(request.sort ?? {}).filter(
    (rule): rule is [keyof typeof supportedSelectors, 'asc' | 'desc'] => (
      rule[0] in supportedSelectors && (rule[1] === 'asc' || rule[1] === 'desc')
    ),
  );
  ...
  return [...entries].sort((left, right) => {
    for (const [field, direction] of sortRules) {
      const result = compareValues(supportedSelectors[field](left), supportedSelectors[field](right), direction);
      if (result !== 0) {
        return result;
      }
    }
...
function buildListResponse(entries: MockAuctionEntry[], request: AuctionListRequest): AuctionListResponseBase {
...
function decimalScale(values: number[]): number {
  const precision = Math.min(
    8,
    Math.max(...values.map((value) => (String(value).split('.')[1] ?? '').length)),
  );
  return 10 ** precision;
}
...
function createUserBet(entry: MockAuctionEntry, price: number): BetItem {
  const nextId = Math.max(0, ...mockStore.auctions.flatMap((auction) => auction.bets.map((bet) => bet.id ?? 0))) + 1;
...
function updateAuctionAfterBid(entry: MockAuctionEntry, price: number): void {
  const existingBet = entry.bets.find(
    (bet) => bet.subscriber_id === CURRENT_USER_SUBSCRIBER_ID && !isHistoricalBet(bet),
  );
  ...
  entry.listItem.main = {
    ...
    price_per_km: entry.detail.cargo.distance
      ? Math.round((currentPriceNoVat / entry.detail.cargo.distance) * 100) / 100
      : 0,
  };
}
...
export const handlers = [
  http.post('/api/v1/auctions/list', async ({ request }) => {
    ...
    if (body !== null && !isRecord(body)) {
      return createValidationResponse('body', 'Тело запроса должно быть объектом.', 'invalid_type');
    }
    ...
    if (listRequest.page !== undefined && (!Number.isInteger(listRequest.page) || listRequest.page < 1)) {
      return createValidationResponse('page', 'Номер страницы должен быть положительным целым числом.', 'min_value');
    }
    if (listRequest.per_page !== undefined && (!Number.isInteger(listRequest.per_page) || listRequest.per_page < 1 || listRequest.per_page > 100)) {
      return createValidationResponse('per_page', 'Размер страницы должен быть от 1 до 100.', 'range');
    }

    const entries = sortEntries(
      mockStore.auctions.filter((entry) => matchesListRequest(entry, listRequest)),
      listRequest,
    );
  ...
  http.get('/api/v1/auctions/:auctionUuid/bets', async ({ params, request }) => {
    ...
    if (entry.detail.trading.hide_bets_history || entry.detail.hide_bets_history) {
      return HttpResponse.json({ bets: [] });
    }

    const includeAll = new URL(request.url).searchParams.get('all') === 'true';
    const bets = includeAll ? entry.bets : entry.bets.filter((bet) => !isHistoricalBet(bet));
    return HttpResponse.json({ bets });
  }),
  ...
  http.post('/api/v1/auctions/:auctionUuid/bets', async ({ params, request }) => {
    ...
    const body = await readJsonBody(request);
    if (!isRecord(body) || typeof body.price !== 'number' || !Number.isFinite(body.price) || body.price <= 0) {
      return createValidationResponse('price', 'Цена должна быть числом больше нуля.', 'positive_number');
    }
    if (!entry.detail.trading.can_set_bet) {
      return createValidationResponse('price', 'Приём ставок для этого аукциона закрыт.', 'bidding_unavailable');
    }

    const constraints = entry.detail.trading.price;
    if (constraints?.min !== null && constraints?.min !== undefined && body.price < constraints.min) {
      return createValidationResponse('price', `Цена должна быть не меньше ${constraints.min}.`, 'min_value');
    }
    if (constraints?.max !== null && constraints?.max !== undefined && body.price > constraints.max) {
      return createValidationResponse('price', `Цена должна быть не больше ${constraints.max}.`, 'max_value');
    }
    if (constraints?.step !== null && constraints?.step !== undefined) {
      ...
      if (!followsStep(body.price, base, constraints.step)) {
        return createValidationResponse('price', `Цена должна соответствовать шагу ${constraints.step}.`, 'step');
      }
    }
