...
async function requestPost(path: string, body: unknown): Promise<Response> {
...
async function requestAuctionList(
  ...
  const response = await requestPost("/auctions/list", request);
...
async function requestAuctionBets(
  auctionUuid: string,
  includeAll = false,
): Promise<BetListResponse> {
  const response = await fetch(
    `${API_URL}/auctions/${auctionUuid}/bets?all=${includeAll}`,
  );
  expect(response.status).toBe(200);
...
describe("MSW mock backend", () => {
  it("filters by the required fields using the shared store", async () => {
    const leading = mockStore.auctions.find(
      (entry) => entry.scenario === "active-leading",
    );
    expect(leading).toBeDefined();

    const response = await requestAuctionList({
      cargo_num: "auc-002",
      status: ["Leading"],
      statuses: [2],
      auc_type: ["Down"],
  ...
  it("returns problem+json for reserved and unknown scenarios", async () => {
    const unauthorized = await requestPost("/auctions/list", {
      cargo_num: "scenario-401",
    });
    const unavailable = await fetch(
      `${API_URL}/auctions/${UNAVAILABLE_AUCTION_UUID}`,
    );
    const notFound = await fetch(
      `${API_URL}/auctions/cccccccc-cccc-4ccc-8ccc-cccccccccccc`,
    );
    ...
    expect(unavailable.headers.get("content-type")).toContain(
      "application/problem+json",
    );
    expect(notFound.headers.get("content-type")).toContain(
      "application/problem+json",
    );
  });

  it("supports all=true and does not expose hidden history", async () => {
    const finished = mockStore.auctions.find(
      (entry) => entry.scenario === "finished-visible-winner",
    );
    const hidden = mockStore.auctions.find(
      (entry) => entry.scenario === "hidden-history",
    );
    expect(finished).toBeDefined();
    expect(hidden).toBeDefined();

    const activeOnly = await requestAuctionBets(finished!.uuid);
    const allBets = await requestAuctionBets(finished!.uuid, true);
    const hiddenBets = await requestAuctionBets(hidden!.uuid, true);
  ...
  it("returns field validation errors for invalid and blocked bids", async () => {
    const active = mockStore.auctions.find(
    ...
    const blocked = await requestPost(`/auctions/${finished!.uuid}/bets`, {
      price: 120_000,
    });
    const blockedProblem = (await blocked.json()) as ValidationProblem;
    expect(blocked.status).toBe(422);
    expect(blockedProblem.errors[0]).toMatchObject({
      field: "price",
      code: "bidding_unavailable",
    });
  });
  ...
  it("adds a bid and updates list, detail, history, price, status, and ranking", async () => {
    ...
    const response = await requestPost(`/auctions/${active!.uuid}/bets`, {
      price: 79_500,
    });
    expect(response.status).toBe(200);
    expect(await response.text()).toBe("");

    const detail = await requestAuctionDetail(active!.uuid);
    const bets = await requestAuctionBets(active!.uuid);
    const list = await requestAuctionList({ cargo_num: "AUC-001" });
    const userBet = bets.bets.find(
  ...
  it("changes the current user bid without adding a duplicate", async () => {
    const leading = mockStore.auctions.find(
      (entry) => entry.scenario === "active-leading",
    );
    expect(leading).toBeDefined();
    ...
    await requestPost(`/auctions/${leading!.uuid}/bets`, { price: 73_500 });
    const changedBets = await requestAuctionBets(leading!.uuid);
    const changedDetail = await requestAuctionDetail(leading!.uuid);

    expect(changedBets.bets).toHaveLength(originalCount);
    expect(
      changedBets.bets.filter(
        (bet) => bet.subscriber_id === CURRENT_USER_SUBSCRIBER_ID,
      ),
    ).toHaveLength(1);
