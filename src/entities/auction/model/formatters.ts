...
export function formatNullableValue(value: string | number | null | undefined): string {
...
export function formatQuantity(
  ...
  return formattedValue === EMPTY_VALUE ? EMPTY_VALUE : `${formattedValue} ${unit}`;
}
...
export function formatMoney(
  ...
  const normalizedCode = currencyCode === null || currencyCode === undefined
    ? undefined
    : String(currencyCode);
  const currency = normalizedCode ? currencyByNumericCode[normalizedCode] : undefined;
...
export function formatDateTime(value: string | null | undefined): string {
  ...
  return isValid(date) ? format(date, 'dd.MM.yyyy, HH:mm', { locale: ru }) : EMPTY_VALUE;
}
